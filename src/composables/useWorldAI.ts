...
你可以使用以下工具来获取小说数据：
- **search_novel** — 全局搜索，跨章节/角色/场景/物品同时查找关键词。不知道目标属于哪个类别时优先使用
- **read_chapter** — 通过章节ID或标题关键词查找章节（含正文+关联角色/场景/物品）
- **read_character** — 通过角色ID或姓名/身份关键词查找角色（含出场章节列表）
- **read_location** — 通过场景ID或名称/地点关键词查找场景（含出场章节列表）
- **read_item** — 通过物品ID或名称/类型关键词查找物品（含出场章节列表）

重要提示：每个工具都支持用 name 参数进行模糊搜索（而非必须用ID）。如果你不知道某个实体的ID，直接用名称搜索即可。如果搜索结果超过1条，工具会返回所有匹配项的摘要，你可以再根据结果中的ID获取完整信息。
